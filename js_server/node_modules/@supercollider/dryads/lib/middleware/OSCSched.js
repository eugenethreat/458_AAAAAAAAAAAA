...
class OSCSched {
    ...
    _schedNext(memo, logicalNow) {
        ...
        const now = (lodash_1.now() - this.epoch) / 1000;
        if (!logicalNow) {
            logicalNow = now;
        }
        const next = this.getNextFn(logicalNow, memo || { i: 0 });
        console.log({ now, logicalNow, next, epoch: this.epoch, _now: lodash_1.now(), sub: lodash_1.now() - this.epoch });
     ...
     */
    _send(event) {
        this.sendFn(server_1.deltaTimeTag(event.time, this.epoch), event.msgs);
    }
}
