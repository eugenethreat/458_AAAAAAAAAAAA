 ...
 * @private
 */
class SclangIO extends events_1.EventEmitter {
    constructor() {
        super();
            ...
            ready: [
                {
                    ...
                    fn: (match, text) => {
                        const guid = match[1], type = match[2], body = match[3];
                        let response, stdout, obj, lines, started = false, stopped = false;
                        switch (type) {
                            case "CAPTURE":
                                if (body === "START") {
                                    this.capturing[guid] = [];
                            ...
                            case "END":
                                ...
                                if (guid in this.calls) {
                                    if (response.type === "Result") {
                                        ...
                                        this.calls[guid].reject(new Errors_1.SCLangError(`Interpret error: ${obj && obj["errorString"]}`, response.type, err || obj));
                                    }
    ...
    parseSyntaxErrors(text) {
        const msgRe = /^ERROR: syntax error, (.+)$/m, msgRe2 = /^ERROR: (.+)$/m, fileRe = /in file '(.+)'/m, lineRe = /line ([0-9]+) char ([0-9]+):$/m;
        const msg = msgRe.exec(text) || msgRe2.exec(text), line = lineRe.exec(text), file = fileRe.exec(text), code = text
            .split("\n")
            .slice(4, -3)
            .join("\n")
            .trim();
    ...
    processOutput() {
        ...
        lodash_1.default.each(parsed, (value, key) => {
