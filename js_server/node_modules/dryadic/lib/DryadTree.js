...
const create_1 = tslib_1.__importDefault(require("lodash/create"));
...
class DryadTree {
     ...
     */
    constructor(root, getClass, rootContext = {}) {
        this.root = root;
        this.dryads = {};
        this.contexts = {};
     ...
     * @returns {CommandNode}
     */
    collectCommands(stateTransitionName, node, player) {
        ...
        return new CommandNode_1.default(commands, context, dryad.properties, node.id, node.children.map(child => this.collectCommands(stateTransitionName, child, player)));
    }
    /**
     ...
     */
    makeCommandTree(nodeId, commands) {
        return new CommandNode_1.default(commands, this.contexts[nodeId], this.dryads[nodeId].properties, nodeId, []);
    }
    /**
    ...
    dryadProperties(nodeId) {
        ...
        return utils_1.mapProperties(dryad.properties, (value) => {
            return isFunction_1.default(value) ? value(context) : value;
        });
    }
    /**
    ...
    updateContext(dryadId, update) {
        return assign_1.default(this.contexts[dryadId], update);
    }
    getContext(dryadId) {
    ...
    getDebugState() {
        ...
        const dbug = node => {
            ...
            if (node.children.length) {
                r = assign_1.default(r, { children: node.children.map(dbug) });
            }
            return r;
        };
        if (this.tree) {
    ...
    _createContext(dryad, dryadId, parentId, rootContext = {}) {
        const cc = assign_1.default({ id: dryadId }, rootContext, dryad.initialContext());
        if (parentId) {
            ...
            if (parentContext) {
                ...
                return create_1.default(parentContext, assign_1.default(childContext, cc));
            }
        }
        return cc;
    }
    ...
    _makeTree(dryad, parentId, childIndex = 0, memo = {}) {
        ...
        if (memo.skipRequireParentOf === dryad) {
            ...
            if (rq) {
                if (!find_1.default(memo.seenTypes, st => st === rq)) {
                    // fetch the parent class from dryadTypes registery by name
                    if (!this.getClass) {
                        throw new Error("A getClass lookup was not provided to DryadTree and " +
                            dryad.constructor.name +
                            " needs one for requireParent()");
                    }
                    const requiredParent = new (this.getClass(rq))({}, [dryad]);
        ...
        const propertiesDryad = Properties_1.invertDryadicProperties(dryad);
        if (propertiesDryad) {
        ...
        const makeSubgraph = (dr) => {
            const subgraph = dr.subgraph();
            if (subgraph) {
                const subMemo = clone_1.default(memo);
        ...
        return new TreeNode(id, dryad, dryadType, dryad.children.map((child, i) => {
